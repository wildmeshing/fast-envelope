...
private:
    ...
    bool triangle_out_simple(
        const std::array<Vector3, 3>& triangle,
        const std::vector<unsigned int>& prismindex) const;

    bool triangle_out_of_envelope(
        const std::array<Vector3, 3>& triangle,
        const std::vector<unsigned int>& prismindex) const;
    bool segment_out_of_envelope(
        const Vector3& seg0,
        const Vector3& seg1,
        const std::vector<unsigned int>& prismindex) const;
    ...
    int Implicit_Seg_Facet_interpoint_Out_Prism_return_local_id_with_face_order(
        ...
        const Vector3& triangle2,
        const std::vector<unsigned int>& prismindex,
        const std::vector<std::vector<int>>& faceorder,
        const int& jump,
        int& id) const;
#ifdef ENVELOPE_WITH_GMP
    ...
    // this function check the adjacent polyhedrons and jump over the polyhedrons that already in
    // the cover list
    ...
    int Implicit_Seg_Facet_interpoint_Out_Prism_return_local_id_with_face_order_jump_over_Rational(
        ...
        int& id) const;
    int
    Implicit_Seg_Facet_interpoint_Out_Prism_return_local_id_with_face_order_jump_over_Multiprecision(
        const Vector3& segpoint0,
        const Vector3& segpoint1,
        const Vector3& triangle0,
        const Vector3& triangle1,
    ...
    int Implicit_Tri_Facet_Facet_interpoint_Out_Prism_return_local_id_with_face_order_Rational(
        ...
        int& id) const;
#endif
    static bool is_3_triangle_cut_pure_multiprecision(
        const std::array<Vector3, 3>& triangle,
    ...
    static int is_3_triangle_cut_float_fast(
        ...
        const Vector3& facet22);
#ifdef ENVELOPE_WITH_GMP
    static bool is_3_triangle_cut_Rational(
        const std::array<Vector3, 3>& triangle,
        const Vector3& facet10,
        const Vector3& facet11,
        const Vector3& facet12,
        ...
        const Vector3& facet22);
#endif
    int is_triangle_cut_envelope_polyhedra(
        const int& cindex,
        const Vector3& tri0,
        const Vector3& tri1,
        const Vector3& tri2,
        std::vector<int>& cid) const;
    bool is_seg_cut_polyhedra(
        const int& cindex,
        const Vector3& seg0,
        const Vector3& seg1,
        std::vector<int>& cid) const;
