...
{
    int o1, o2;
    o1 = orient_3d(seg0, t0, t1, t2);
    o2 = orient_3d(seg1, t0, t1, t2);
    int op = o1 * o2;
    if (op >= 0) {
    ...
    const Vector3& triangle2)
{
    ...
    if (triangle0[0] != triangle1[0] || triangle0[1] != triangle1[1] ||
        triangle0[2] != triangle1[2]) {
        return DEGENERATED_SEGMENT;
    }
    if (triangle0[0] != triangle2[0] || triangle0[1] != triangle2[1] ||
        triangle0[2] != triangle2[2]) {
        return DEGENERATED_SEGMENT;
    }
...
Vector3 accurate_cross_product_direction(
    const Vector3& p0,
    const Vector3& p1,
    const Vector3& q0,
    const Vector3& q1)
    ...
    const Scalar& epsilon)
{
    ...
    const auto get_triangle_obtuse_angle =
    ...
    const auto get_corner_plane = [](const Vector3& p0,
                                     const Vector3& midp,
        ...
        Scalar distance_small =
            distance *
            1; // to be conservative to reduce numerical error, can set the Scalar as 0.999
        ...
        Vector3 axis;
        if (use_exact)
            axis = accurate_cross_product_direction(midp, p0, origin, normal);
        else
    ...
    for (int i = 0; i < m_faces.size(); i++) {
        ...
        int obtuse = get_triangle_obtuse_angle(
            m_ver[m_faces[i][0]],
            m_ver[m_faces[i][1]],
            m_ver[m_faces[i][2]]);
        ...
        if (use_accurate_cross)
            edgenormaldist =
                accurate_cross_product_direction(origin, edgedire, origin, normal) * tolerance;
        else
        ...
        if (obtuse != 1) {
            get_corner_plane(
                m_ver[m_faces[i][1]],
                (m_ver[m_faces[i][0]] + m_ver[m_faces[i][2]]) / 2,
                normal,
                tolerance,
                plane[0],
                plane[1],
        ...
        if (obtuse != 2) {
            get_corner_plane(
                m_ver[m_faces[i][2]],
                (m_ver[m_faces[i][0]] + m_ver[m_faces[i][1]]) / 2,
                normal,
                tolerance,
                plane[0],
                plane[1],
        ...
        if (obtuse != 0) {
            get_corner_plane(
                m_ver[m_faces[i][0]],
                (m_ver[m_faces[i][1]] + m_ver[m_faces[i][2]]) / 2,
                normal,
                tolerance,
                plane[0],
                plane[1],
    ...
    const std::vector<Scalar>& epsilons)
{
    ...
    for (int i = 0; i < m_faces.size(); i++) {
        ...
        if (use_accurate_cross)
            edgenormaldist =
                accurate_cross_product_direction(origin, edgedire, origin, normal) * tolerance[i];
        else
            edgenormaldist = edgedire.cross(normal).normalized() * tolerance[i];
        plane[0] = m_ver[m_faces[i][0]] + edgenormaldist;
        plane[1] = m_ver[m_faces[i][1]] + edgenormaldist;
        plane[2] = plane[0] + normal;
        halfspace[i].emplace_back(plane); // number 2
